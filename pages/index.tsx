    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        <PopoverTransition show={menuOpen} style={{ position: "absolute", right: 0, top: "calc(100% + 4px)", zIndex: 50 }}>
          ...
          >
            <button onClick={() => { setMenuOpen(false); setShowEditModal(true); }} style={{ display: "block", width: "100%", textAlign: "left", padding: "9px 14px", border: "none", background: "#fff", fontSize: 13, color: "#121216", fontFamily: "var(--font-inter)", cursor: "pointer" }}>Edit Activity</button>
            <div style={{ height: 1, background: "#E5E5EA" }} />
      ...
      <DeleteActivityModal
        ...
        onConfirm={onDelete}
      />
      <EditActivityModal activity={item} show={showEditModal} onClose={() => setShowEditModal(false)} />
    </div>
  );
}
...
// ─── Edit Activity Modal ──────────────────────────────────────────────────────
function EditActivityModal({ activity, show, onClose }: { activity: ActivityItem | null; show: boolean; onClose: () => void }) {
  const [mounted,     setMounted]     = useState(show);
  const [vis,         setVis]         = useState(show);
  const [title,       setTitle]       = useState("");
  const [description, setDescription] = useState("");
  ...
  useEffect(() => {
    if (show && activity) {
      ...
      const tab: AssignTab =
        activity.assignedTo.type === "all"   ? "All Students" :
        activity.assignedTo.type === "staff" ? "Group" : "Individual Students";
      setAssignTab(tab);
      if (activity.assignedTo.type === "students") {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div style={{ padding: "24px 24px 20px", flexShrink: 0 }}>
          <p style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#121216" }}>Edit Activity</p>
        </div>
        <div style={{ height: 1, background: "#E8E8EC", flexShrink: 0 }} />
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div>
            <p style={labelStyle}>Description</p>
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.5 }} />
          </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              {dueDate && <span style={{ fontSize: 13, color: "#8E8E97" }}>Series will automatically end after 5 months.</span>}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", background: "#F3F3F7", borderRadius: 10, padding: 3, marginBottom: 10 }}>
              {ASSIGN_TABS.map(tab => (
                <button key={tab} onClick={() => setAssignTab(tab)} style={{ border: "none", borderRadius: 8, padding: "7px 0", background: assignTab === tab ? "#fff" : "transparent", boxShadow: assignTab === tab ? "0 1px 4px rgba(0,0,0,0.10)" : "none", fontSize: 13, fontWeight: assignTab === tab ? 600 : 400, color: assignTab === tab ? "#121216" : "#8E8E97", fontFamily: "var(--font-inter)", cursor: "pointer", transition: `background ${MS.dFast} ${MS.eOut}, box-shadow ${MS.dFast} ${MS.eOut}` }}>
                  {tab}
                </button>
              ))}
            </div>
            {assignTab === "Individual Students" && (
              <>
                <div onClick={() => inputRef.current?.focus()} style={{ minHeight: 44, padding: "6px 10px", borderRadius: 8, border: "1.5px solid #3E4FD3", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 6, cursor: "text", marginBottom: 10 }}>
                  {selectedStudents.map(s => (
                    <span key={s.id} style={{ display: "inline-flex", alignItems: "center", gap: 5, height: 26, paddingInline: 9, borderRadius: 20, background: hexAlpha("#3E4FD3", 0.1), fontSize: 12, fontWeight: 500, color: "#3E4FD3" }}>
                      {chipLabel(s.name)}
                      <button onClick={e => { e.stopPropagation(); toggle(s.id); }} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", display: "flex", alignItems: "center", color: "#3E4FD3" }}>
                  ...
                  <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)} placeholder={selectedStudents.length === 0 ? "+ Add students..." : ""} style={{ border: "none", outline: "none", flex: 1, minWidth: 100, fontSize: 13, color: "#121216", fontFamily: "var(--font-inter)", background: "transparent", padding: "2px 0" }} />
                </div>
                <div style={{ border: BORDER, borderRadius: 8, maxHeight: 180, overflowY: "auto" }}>
                  {filtered.length === 0
                    ? <div style={{ padding: "12px 14px", fontSize: 13, color: "#A0A0AA" }}>No students found.</div>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {subTasks.map(st => (
                <div key={st.id} style={{ display: "flex", alignItems: "center", gap: 8, border: BORDER, borderRadius: 8, padding: "8px 10px", background: "#fff" }}>
                  ...
                  <input value={st.title} onChange={e => updateSubTask(st.id, "title", e.target.value)} placeholder="Sub-task title" style={{ flex: 1, border: "none", outline: "none", fontSize: 13, color: "#121216", fontFamily: "var(--font-inter)", background: "transparent" }} />
                  <div style={{ display: "flex", alignItems: "center", gap: 4, background: "#F0F0F5", borderRadius: 6, padding: "3px 8px", flexShrink: 0 }}>
                    <input type="number" min={1} value={st.duration} onChange={e => updateSubTask(st.id, "duration", Math.max(1, parseInt(e.target.value) || 1))} style={{ width: 28, border: "none", outline: "none", background: "transparent", fontSize: 12, color: "#4A4A55", fontFamily: "var(--font-inter)", textAlign: "right", padding: 0 }} />
                    <span style={{ fontSize: 12, color: "#4A4A55" }}>min</span>
                    <button onClick={() => removeSubTask(st.id)} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", display: "flex", alignItems: "center", color: "#8E8E97", marginLeft: 2 }}>
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"><path d="M1.5 1.5l7 7M8.5 1.5l-7 7"/></svg>
                    </button>
            ...
            <button onClick={addSubTask} style={{ marginTop: 8, height: 34, paddingInline: 14, borderRadius: 8, border: BORDER, background: "#fff", fontSize: 13, fontWeight: 500, color: "#4A4A55", fontFamily: "var(--font-inter)", cursor: "pointer" }}>
              + Add sub-task
            </button>
          </div>
        ...
        <div style={{ padding: "16px 24px", display: "flex", justifyContent: "flex-end", gap: 8, flexShrink: 0 }}>
          <button onClick={onClose} style={{ height: 36, paddingInline: 16, borderRadius: 8, border: BORDER, background: "#fff", color: "#121216", fontSize: 14, fontWeight: 500, fontFamily: "var(--font-inter)", cursor: "pointer" }}>Cancel</button>
          <button onClick={onClose} style={{ height: 36, paddingInline: 16, borderRadius: 8, border: "none", background: "#3E4FD3", color: "#fff", fontSize: 14, fontWeight: 700, fontFamily: "var(--font-inter)", cursor: "pointer" }}>Save Changes</button>
        </div>
      </div>
    </div>
  );
...
function ActivitiesPage({ onOpenActivity }: { onOpenActivity: (id: number) => void }) {
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                        ...
                        <div style={{ paddingInline: 4, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                          ...
                          <PopoverTransition show={menuOpen} style={{ position: "absolute", right: 0, top: "calc(100% + 2px)", zIndex: 50 }}>
                            ...
                            >
                              ...
                              <button onClick={() => { setEditTarget(item); setOpenMenu(null); }} style={actMenuItemStyle}>Edit</button>
                              <div style={{ height: 1, background: "#E5E5EA" }} />
      ...
      <EditActivityModal activity={editTarget} show={editTarget !== null} onClose={() => setEditTarget(null)} />
      <Toast message={toastMsg} show={toastShow} />
    </div>
  );
}
